...
use App\Libs\Auth\Role;
use App\Libs\Auth\Status ;
use Session;
use App\Libs\Auth\UserManager ;
use App\Libs\Auth\Exceptions\InvalidPasswordException ;
use App\Libs\Auth\Exceptions\InvalidEmailException ;
use App\Libs\Auth\Exceptions\EmailNotVerifiedException ;
use App\Libs\Auth\Exceptions\TooManyRequestsException ;
use App\Libs\Auth\Exceptions\NotLoggedInException ;
use App\Libs\Auth\Exceptions\InvalidSelectorTokenPairException ;
use App\Libs\Auth\Exceptions\HeadersAlreadySentError ;
use App\Libs\Auth\Exceptions\ConfirmationRequestNotFound ;
...
final class Auth extends UserManager
{
        ...
        if (!$this->isLoggedIn()) {
            ...
            if (isset($_COOKIE[$this->rememberCookieName])) {
                ...
                if (!empty($parts[0]) && !empty($parts[1])) {
                    try {
                        DB::table(config('auth.tables.users_remembered')." as a");
                        DB::select('a.user, a.token, a.expires, b.email, b.username, b.status, b.roles_mask, b.force_logout');
                        DB::leftJoin(config('auth.tables.users')." as b", "a.user = b.id ");
                        DB::where('a.selector', '=',  $parts[0]);
                        $rememberData = DB::getRow("array");
                    } catch (Error $e) {
		...
		if ($this->isLoggedIn()) {
			...
			if (($_SESSION[self::SESSION_FIELD_LAST_RESYNC] + $this->sessionResyncInterval) <= \time()) {
				...
				try {
                    ...
                    DB::select('email, username, status, roles_mask, force_logout');
                    DB::where('id','=',$this->getUserId());


					$authoritativeData = DB::getRow('array');
				}
				catch (Error $e) {
    ...
    public function confirmEmail($selector, $token)
    {
        ...
        try {
            DB::table(config('auth.tables.users_confirmations')." as a");
            DB::select('a.id, a.user_id, a.email AS new_email, a.token, a.expires, b.email AS old_email');
            DB::leftJoin("users as b", "b.id = a.user_id");
            DB::where('a.selector', '=', $selector);
            $confirmationData = DB::getRow("array");
        } catch (Error $e) {
            throw new DatabaseError();
        }
        if (!empty($confirmationData)) {
            if (\password_verify($token, $confirmationData['token'])) {
                if ($confirmationData['expires'] >= \time()) {
                    ...
                    try {
                        DB::table(config('auth.tables.users_resets'));
                        DB::where('user', '=', $confirmationData['user_id']);
                        DB::delete();
                    } catch (Error $e) {
                        throw new DatabaseError();
                    }
                    ...
                    try {
                        DB::table(config('auth.tables.users'));
                        DB::where('id', '=', $confirmationData['user_id']);
                        DB::update([
                    ...
                    try {
                        DB::table(config('auth.tables.users'));
                        DB::where('id', '=', $confirmationData["id"]);
                        DB::delete();
                    } catch (Error $e) {
                        throw new DatabaseError();
                    }
    ...
    public function changeEmail($newEmail, callable $callback)
    {
        if ($this->isLoggedIn()) {
            ...
            try {
                DB::table(config('auth.tables.users'));
                DB::select('COUNT(*)');
                DB::where('email', '=', $newEmail);
                $existingUsersWithNewEmail = DB::getRow("array")[0];
            } catch (Error $e) {
                throw new DatabaseError();
            }
    ...
    private function resendConfirmationForColumnValue($columnName, $columnValue, callable $callback)
    {
        try {
            ...
            DB::select('user_id,email');
            DB::where($columnName, '=', $columnValue);
            DB::orderBy('id', 'desc');
            DB::limit(0, 1);
            $latestAttempt = DB::getRow("array");
        ...
        if (\password_verify($password, $userData['password'])) {
            ...
            if ((int) $userData['verified'] === 1) {
                if (!isset($onBeforeSuccess) || (\is_callable($onBeforeSuccess) && $onBeforeSuccess($userData['id']) === true)) {
                    ...
                    throw new ExceptionHandler('Attempt Cancelled Exception');
                }
            } else {
                throw new ExceptionHandler('Email Not Verified', $userData);
            }
        } else {
            $this->throttle([ 'attemptToLogin', $this->getIpAddress() ], 4, (60 * 60), 5, false);
    ...
    private function getOpenPasswordResetRequests($userId)
    {
        try {
            ...
            DB::where('expires', '=', \time());
            $requests =DB::getRow("array");
    ...
    public function resetPassword($selector, $token, $newPassword)
    {
        ...
        try {
            DB::table(config('auth.tables.users_resets')." as a");
            DB::select('a.id, a.user, a.token, a.expires, b.resettable');
            DB::leftJoin(config("auth.tables.users")." as b", "b.id = a.user");
            DB::where('a.selector', '=', $selector);
            $resetData = DB::getRow("array");
    ...
    public function setPasswordResetEnabled($enabled)
    {
        ...
        if ($this->isLoggedIn()) {
            try {
                DB::table(config('auth.tables.users'));
                DB::where('id', '=', $this->getUserId());
                DB::update(['resettable' => $enabled ? 1 : 0]);
            } catch (Error $e) {
                throw new DatabaseError();
            }
        } else {
    ...
    public function isPasswordResetEnabled()
    {
        if ($this->isLoggedIn()) {
            try {
                DB::table(config('auth.tables.users'));
                DB::select('resettable');
                DB::where('id', '=', $this->getUserId());
                $enabled =DB::getRow("array");
    ...
    public function throttle(array $criteria, $supply, $interval, $burstiness = null, $simulated = null, $cost = null)
    {
        ...
        try {
            DB::table(config('auth.tables.users_throttling'));
            DB::select('tokens,replenished_at');
            DB::where('bucket', '=', $key);

            $bucket = DB::getRow("array");
        } catch (Error $e) {
            throw new DatabaseError();
        }
    ...
    private function getRememberDirectiveExpiry()
    {
        ...
        if ($this->isLoggedIn()) {
            ...
            if (isset($existingSelector)) {
                ...
                DB::table(config('auth.tables.users_remembered'));
                DB::where('selector', '=', $existingSelector);
                DB::where('user', '=', $this->getUserId());
                $existingExpiry = DB::getRow('array');
