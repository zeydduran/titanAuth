...
use App\Libs\Auth\Exceptions\UnknownUsernameException ;
...
final class Administration extends UserManager {
	...
	public function doesUserHaveRole($userId, $role) {
		...
		DB::select('roles_mask');
		DB::where('id','=',$userId);
		$rolesBitmask = DB::getRow();
		...
		return ($rolesBitmask->roles_mask & $role) === $role;
	}
	...
	private function modifyRolesForUserByColumnValue($columnName, $columnValue, callable $modification) {
		...
		try {
			...
			DB::update(['roles_mask'=>$newRolesBitmask]);
	...
	private function logInAsUserByColumnValue($columnName, $columnValue) {
		try {
			DB::table(config('auth.tables.users'));
			DB::select('verified, id, email, username, status, roles_mask');
			DB::where($columnName,'=',$columnValue);
			DB::limit(0,2);
